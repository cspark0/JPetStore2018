...
public class RestfulOrderController {
	private OrderService petStoreSvc;

	@Autowired
	public void setPetStoreSvc(OrderService petStoreSvc) {
		this.petStoreSvc = petStoreSvc;
	}
	...
	public List<Order> getOrdersByUsername(@PathVariable("username") String username, HttpServletResponse response)
		...
		List<Order> orderList = this.petStoreSvc.getOrdersByUsername(username);
		if (orderList == null) {
			response.sendError(HttpServletResponse.SC_NOT_FOUND);
			return null;
		}
